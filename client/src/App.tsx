...
class App extends React.Component {
        ...
        <Layout>
          <div className="row">
            <div className="top-left col">
              {orderItems} 
              <button type="submit" className="btn btn-primary" onClick={(event) => this.handleSubmitButton(event)}>Order</button>
              <br/>
              <div className="Add-Remove-Product">
                <input className="Product-Change" type="text" onChange={e => this.handleChangeNewProduct(e.target.value)}/>
                <button type="submit" className="Add-Button" onClick={(event) => this.handleAddItemToList(event)}>Add Product</button>
                <button type="submit" className="Remove-Button" onClick={(event) => this.handleRemoveItemFromList(event)}>Remove Product</button>
              </div>
            </div>
  ...
  handleChangeCart = (event, index : number) => {
    ...
    const newArr = [...this.state.Items.map((item, index2) => {
  ...
  */
  ...
  handleDeleteOrder = (event, index) => {
    const newOrderHistory = this.state.OrderHistory.filter((order : any[], index2) => {
      if(index === index2) {
